/*! $.toggleSlider
 ...
 @documentation http://refreshless.com/toggleslider/
*/
...
(function( $ ){
	...
	$.fn['toggleSlider'] = function( options, rebuild ){
		...
		/*  8 */ ,'-upper'
		/*  9 */ ,'toggle-connect'
		/* 10 */ ,'toggle-horizontal'
		/* 11 */ ,'toggle-vertical'
		/* 12 */ ,'toggle-background'
		/* 13 */ ,'toggle-stacking'
		/* 14 */ ,'toggle-block'
		/* 15 */ ,'toggle-state-blocked'
		/* 16 */ ,'toggle-ltr'
		/* 17 */ ,'toggle-rtl'
		/* 18 */ ,'toggle-dragable'
		/* 19 */ ,'toggle-extended'
		/* 20 */ ,'toggle-state-drag'
		]
...
// Event abstraction
	...
	// Provide a clean event with standardized offset values.
		function fixEvent ( e ) {
			...
			if ( touch ) {
				// toggleSlider supports one movement at a time,
	...
	// Handler for attaching events trough a proxy
		function attach ( events, element, callback, pass ) {
			...
			// Add the toggleSlider namespace to all events.
			...
			return element.on( events, function( e ){
				...
				// 'tap' transition.
				if( target.hasClass('toggle-state-tap') || disabled ) {
					return false;
				}
...
// Serialization and value storage
	...
	// Map serialization to [ element, method ]. Attach events where required.
		function storeElement ( handle, item, number ) {
			...
			// Append a new input to the toggleSlider base.
...
// API
			...
			$.each( tests, function( name, test ){
				...
				// If the test returns false, fail.
					...
					if( console && console.log && console.group ){
						console.group( 'Invalid toggleSlider initialisation:' );
					...
					throw new RangeError('toggleSlider');
				}
			});
		}
	...
	// Unbind all attached events, remove classed and HTML.
		function destroy ( target ) {
			...
			// Remove all events added by toggleSlider.
	...
	// with a simple handler. toggleSlider will use the internal
	// value method, anything else will use the standard method.
		$.fn.val = function(){
			...
			// If this isn't toggleSlider, continue with jQuery's
			// original method.
